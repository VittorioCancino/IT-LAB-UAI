...
export const sendHeartbeat = async (): Promise<void> => {
  ...
  try {
    const response = await axios.post(
      `${INSTANCE_CONFIG.mainServerUrl}/instance/create-instance`,
      {
        instanceId: INSTANCE_CONFIG.instanceId,
        name: INSTANCE_CONFIG.name,
        route: INSTANCE_CONFIG.route,
    ...
    console.log(`[Heartbeat] Registrado exitosamente en servidor principal`);
    console.log(
      `[Heartbeat] Instancia: ${INSTANCE_CONFIG.instanceId} (${INSTANCE_CONFIG.name}:${INSTANCE_CONFIG.route})`,
    );
    ...
    console.error(
      `[Heartbeat] Error al registrar en servidor principal ${INSTANCE_CONFIG.mainServerUrl}/instance/create-instance`,
      errorMessage,
    );
    console.log(
      `[Heartbeat] Instancia: ${INSTANCE_CONFIG.instanceId} (${INSTANCE_CONFIG.name}:${INSTANCE_CONFIG.route})`,
...
export const startHeartbeat = (): void => {
  ...
  console.log(`[Heartbeat] Puerto de esta instancia: ${INSTANCE_CONFIG.route}`);
