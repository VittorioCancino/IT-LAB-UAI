...
export const validateInstanceConfig = (
  ...
  if (!route || !route.startsWith("/") || route.length < 2) {
    errors.push("La ruta debe ser de tipo /route");
  }

  if (!config.mainServerUrl || !config.mainServerUrl.startsWith("http")) {
    errors.push("mainServerUrl debe ser una URL válida que comience con http");
  }
...
export const getCurrentConfig = (): InstanceConfiguration => {
  ...
  return {
    ...INSTANCE_CONFIG,
    route: INSTANCE_CONFIG.route, // Asegurar que el puerto sea número
  };
};
...
console.log(`[Instance Config] Puerto: ${INSTANCE_CONFIG.route}`);
console.log(
